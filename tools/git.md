...
git config --global alias.log-p 'log --pretty=format:"%h %ad %cn %s" -20'
```
...
git config --global alias.log-p 'log --pretty=format:"%h %ad %cn %s"'
git log --pretty=format:"%h %ad %cn %s" --date=short -3 --name-status
```
